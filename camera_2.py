...
def get_pointcloud(color_img_1, depth_img_1, camera_intrinsics_1):

    # Get depth image size
    im_h = depth_img_1.shape[0]
    im_w = depth_img_1.shape[1]

    # Project depth into 3D point cloud in camera coordinates
    pix_x,pix_y = np.meshgrid(np.linspace(0,im_w-1,im_w), np.linspace(0,im_h-1,im_h))
    cam_pts_x = np.multiply(pix_x-camera_intrinsics_1[0][2],depth_img_1/camera_intrinsics_1[0][0])
    cam_pts_y = np.multiply(pix_y-camera_intrinsics_1[1][2],depth_img_1/camera_intrinsics_1[1][1])
    cam_pts_z = depth_img_1.copy()
    cam_pts_x.shape = (im_h*im_w,1)
    cam_pts_y.shape = (im_h*im_w,1)
    cam_pts_z.shape = (im_h*im_w,1)

    # Reshape image into colors for 3D point cloud
    rgb_pts_r = color_img_1[:,:,0]
    rgb_pts_g = color_img_1[:,:,1]
    rgb_pts_b = color_img_1[:,:,2]
    rgb_pts_r.shape = (im_h*im_w,1)
    rgb_pts_g.shape = (im_h*im_w,1)
    rgb_pts_b.shape = (im_h*im_w,1)

    cam_pts_1 = np.concatenate((cam_pts_x, cam_pts_y, cam_pts_z), axis=1)
    rgb_pts_1 = np.concatenate((rgb_pts_r, rgb_pts_g, rgb_pts_b), axis=1)

    return cam_pts_1, rgb_pts_1


def get_heightmap(color_img_1, depth_img_1, cam_intrinsics_1, cam_pose_1, workspace_limits, heightmap_resolution):
    ...
    surface_pts, color_pts = get_pointcloud(color_img_1, depth_img_1, cam_intrinsics_1)

    # Transform 3D point cloud from camera coordinates to robot coordinates
    surface_pts = np.transpose(np.dot(cam_pose_1[0:3,0:3],np.transpose(surface_pts)) + np.tile(cam_pose_1[0:3,3:],(1,surface_pts.shape[0])))
    ...
    depth_heightmap_1 = np.zeros(heightmap_size)
    heightmap_pix_x = np.floor((surface_pts[:,0] - workspace_limits[0][0])/heightmap_resolution).astype(int)
    heightmap_pix_y = np.floor((surface_pts[:,1] - workspace_limits[1][0])/heightmap_resolution).astype(int)
    color_heightmap_r[heightmap_pix_y,heightmap_pix_x] = color_pts[:,[0]]
    color_heightmap_g[heightmap_pix_y,heightmap_pix_x] = color_pts[:,[1]]
    color_heightmap_b[heightmap_pix_y,heightmap_pix_x] = color_pts[:,[2]]
    color_heightmap_1 = np.concatenate((color_heightmap_r, color_heightmap_g, color_heightmap_b), axis=2)
    depth_heightmap_1[heightmap_pix_y,heightmap_pix_x] = surface_pts[:,2]
    z_bottom = workspace_limits[2][0]
    depth_heightmap_1 = depth_heightmap_1 - z_bottom
    depth_heightmap_1[depth_heightmap_1 < 0] = 0
    depth_heightmap_1[depth_heightmap_1 == -z_bottom] = np.nan

    return color_heightmap_1, depth_heightmap_1
...
def get_affordance_vis_1(grasp_affordances, input_images_1, num_rotations, best_pix_ind):
    vis = None
    for vis_row in range(num_rotations/4):
        ...
        for vis_col in range(4):
            ...
            input_image_vis = (input_images_1[rotate_idx,:,:,:]*255).astype(np.uint8)
...
def get_difference(color_heightmap_1, color_space, bg_color_heightmap):
    ...
    color_space = np.tile(color_space, (1, color_heightmap_1.shape[0], color_heightmap_1.shape[1], 1))

    # Normalize color heightmaps
    color_heightmap_1 = color_heightmap_1.astype(float)/255.0
    color_heightmap_1.shape = (1, color_heightmap_1.shape[0], color_heightmap_1.shape[1], color_heightmap_1.shape[2])
    color_heightmap_1 = np.tile(color_heightmap_1, (color_space.shape[0], 1, 1, 1))
    ...
    key_color_dist = np.sqrt(np.sum(np.power(color_heightmap_1 - color_space,2), axis=3))
